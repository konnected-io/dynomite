...
describe Dynomite::Item do
  before(:each) do
    allow(CallbacksTester).to receive(:partition_key).and_return(:id)
    allow(CallbacksTester).to receive(:sort_key).and_return(nil)
    allow(CallbacksTester).to receive(:attribute_definitions).and_return([]) # avoid typcast
  end

  let(:model_klass) do
    klass = CallbacksTester
    allow(klass.client).to receive(:put_item).and_return(null)
    klass
  end
  ...
  it "destroy" do
    model = model_klass.new
    expect(model_klass.client).to receive(:delete_item)
