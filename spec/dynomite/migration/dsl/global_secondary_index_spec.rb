...
describe GSI do
  def dsl
    double(:dsl, :namespaced_table_name=>"posts", :partition_key_identifier=>"id:string", :sort_key_identifier=>"created_at:string")
  end
  ...
  context "update index" do
    let(:index) do
      allow(dsl).to receive(:client).and_return(double(:client).as_null_object)
      GSI.new(dsl, :update) do |i|
  ...
  context "delete index" do
    let(:index) do
      allow(dsl).to receive(:client).and_return(double(:client).as_null_object)
      GSI.new(dsl, :delete, "delete-me-index")
    end
